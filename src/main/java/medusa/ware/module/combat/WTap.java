...
public class WTap extends Module
{
    ...
    @SubscribeEvent
    public void t3s4f(final TickEvent.RenderTickEvent renderTickEvent) {
    ...
    public void asdfgbnv(final AttackEntityEvent attackEntityEvent) {
        if (this.tDelay.hasReached(this.delay) && !this.shouldTap && WTap.mc.objectMouseOver != null && WTap.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY && WTap.mc.objectMouseOver.entityHit.getEntityId() == attackEntityEvent.target.getEntityId() && CombatUtils.canTarget(Game.World().getEntityByID(attackEntityEvent.target.getEntityId()))) {
            this.shouldTap = true;
            this.tHold.reset();
            this.tDelay.reset();
            KeyBinding.setKeyBindState(WTap.mc.gameSettings.keyBindForward.getKeyCode(), false);
